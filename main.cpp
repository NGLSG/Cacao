...
int main()
{
    try
    {
        ...
        instanceCreateInfo.type = BackendType::DirectX12;
        ...
        if (d3d12Device)
        {
            HRESULT status = d3d12Device->GetHandle()->GetDeviceRemovedReason();
            std::cout << "Device status before Write Descriptor: 0x" << std::hex << status << std::endl;
        }
        for (uint32_t i = 0; i < BUFFER_COUNT; i++)
        {
            ...
            {
                HRESULT status = d3d12Device->GetHandle()->GetDeviceRemovedReason();
                std::cout << "Device status before Write Buffer: 0x" << std::hex << status << std::endl;
            }
            ...
            {
                HRESULT status = d3d12Device->GetHandle()->GetDeviceRemovedReason();
                std::cout << "Device status before  Write Texture: 0x" << std::hex << status << std::endl;
            }

            descriptorSets[i]->WriteTexture({
                1, spriteTexture->GetDefaultView(), ImageLayout::ShaderReadOnly, DescriptorType::SampledImage, nullptr,
                0
            });
            if (d3d12Device)
            {
                HRESULT status = d3d12Device->GetHandle()->GetDeviceRemovedReason();
                std::cout << "Device status before  Write Sampler: 0x" << std::hex << status << std::endl;
            }
        ...
        {
            HRESULT status = d3d12Device->GetHandle()->GetDeviceRemovedReason();
            std::cout << "Device status before Shader Compile: 0x" << std::hex << status << std::endl;
        }
        ...
        {
            HRESULT status = d3d12Device->GetHandle()->GetDeviceRemovedReason();
            std::cout << "Device status before PipelineLayout: 0x" << std::hex << status << std::endl;
        }
        ...
        std::cout << "Rendering " << SPRITE_COUNT << " sprites with " << NUM_THREADS << " update threads..." <<
            std::endl;
        ...
        while (isRunning.load())
        {
            ...
            colorAttachment.ClearValue = ClearValue::ColorFloat(1.f, 1.f, 1.0f, 1.0f);

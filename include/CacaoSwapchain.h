...
namespace Cacao
{
    ...
    class CACAO_API CacaoSwapchain : public std::enable_shared_from_this<CacaoSwapchain>
    {
    public:
        virtual ~CacaoSwapchain() = default;
        virtual Result Present(
            const Ref<CacaoQueue>& queue,
            const Ref<CacaoSynchronization>& sync,
            uint32_t frameIndex) = 0;
        virtual uint32_t GetImageCount() const = 0;
        virtual Ref<CacaoTexture> GetBackBuffer(uint32_t index) const = 0;

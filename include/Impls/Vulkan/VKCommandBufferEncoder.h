...
namespace Cacao
{
    ...
    public:
        ...
        void BindGraphicsPipeline(const std::shared_ptr<CacaoGraphicsPipeline>& pipeline) override;
        void BindComputePipeline(const std::shared_ptr<CacaoComputePipeline>& pipeline) override;
        void SetViewport(const Viewport& viewport) override;
        void SetScissor(const Rect2D& scissor) override;
        void BindVertexBuffer(uint32_t binding, const std::shared_ptr<CacaoBuffer>& buffer, uint64_t offset) override;
        void BindIndexBuffer(const std::shared_ptr<CacaoBuffer>& buffer, uint64_t offset, IndexType indexType) override;
        void BindDescriptorSets(const std::shared_ptr<CacaoGraphicsPipeline>& pipeline, uint32_t firstSet,
                                const std::vector<std::shared_ptr<CacaoDescriptorSet>>& descriptorSets) override;
        void PushConstants(const std::shared_ptr<CacaoGraphicsPipeline>& pipeline, ShaderStageFlags stageFlags,
